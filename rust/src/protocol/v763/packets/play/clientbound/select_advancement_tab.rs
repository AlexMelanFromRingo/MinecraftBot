...
impl ClientboundPacket for SelectAdvancementTab {
    ...
    fn encode(&self, writer: &mut BytesWriter) -> Result<(), ProtocolError> {
        match &self.id { None => writer.write_all(&[0]),
            Some(s) => { writer.write_all(&[1])?; string::write(s, writer) } }
    }
}
