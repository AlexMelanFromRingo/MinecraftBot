...
impl Login {
    pub fn decode(reader: &mut BytesReader<'_>) -> Result<Self, ProtocolError> {
        ...
        for _ in 0..n_worlds { world_names.push(identifier::read(reader)?); }
        ...
        Ok(Self {
            entity_id, is_hardcore, game_mode, previous_game_mode, world_names,
            dimension_codec, world_type, world_name, hashed_seed, max_players,
            view_distance, simulation_distance, reduced_debug_info,
            enable_respawn_screen, is_debug, is_flat, death_location, portal_cooldown,
        })
    }
}
...
impl ClientboundPacket for Login {
    ...
    fn encode(&self, writer: &mut BytesWriter) -> Result<(), ProtocolError> {
        ...
        for w in &self.world_names { identifier::write(w, writer)?; }
        ...
        for b in [self.reduced_debug_info, self.enable_respawn_screen, self.is_debug, self.is_flat] {
