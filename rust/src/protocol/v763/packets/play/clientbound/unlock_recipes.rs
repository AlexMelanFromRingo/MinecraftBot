...
impl UnlockRecipes {
    pub fn decode(reader: &mut BytesReader<'_>) -> Result<Self, ProtocolError> {
        ...
        for _ in 0..n1 {
            recipe_ids_1.push(identifier::read(reader)?);
        }
        ...
        if action == 0 {
            ...
            for _ in 0..n2 {
                recipe_ids_2.push(identifier::read(reader)?);
            }
        }
        Ok(Self {
            action,
            crafting_book_open,
            crafting_book_filter_active,
            smelting_book_open,
            smelting_book_filter_active,
            blast_furnace_book_open,
            blast_furnace_book_filter_active,
            smoker_book_open,
            smoker_book_filter_active,
            recipe_ids_1,
            recipe_ids_2,
        })
    }
...
impl ClientboundPacket for UnlockRecipes {
    ...
    fn encode(&self, writer: &mut BytesWriter) -> Result<(), ProtocolError> {
        ...
        for b in [
            ...
            self.crafting_book_filter_active,
            self.smelting_book_open,
            self.smelting_book_filter_active,
            self.blast_furnace_book_open,
            self.blast_furnace_book_filter_active,
            self.smoker_book_open,
            self.smoker_book_filter_active,
        ] {
            writer.write_all(&[if b { 1 } else { 0 }])?;
        }
        varint::write(self.recipe_ids_1.len() as i32, writer)?;
        for r in &self.recipe_ids_1 {
            identifier::write(r, writer)?;
        }
        if self.action == 0 {
            varint::write(self.recipe_ids_2.len() as i32, writer)?;
            for r in &self.recipe_ids_2 {
                identifier::write(r, writer)?;
            }
        }
        Ok(())
