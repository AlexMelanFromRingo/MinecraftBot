...
impl StopSound {
    pub fn decode(reader: &mut BytesReader<'_>) -> Result<Self, ProtocolError> {
        ...
        let source = if flags & 0x01 != 0 { Some(varint::read(reader)?) } else { None };
        let sound = if flags & 0x02 != 0 { Some(string::read(reader)?) } else { None };
...
impl ClientboundPacket for StopSound {
    ...
    fn encode(&self, writer: &mut BytesWriter) -> Result<(), ProtocolError> {
        ...
        if let Some(s) = self.source { varint::write(s, writer)?; }
        if let Some(s) = &self.sound { string::write(s, writer)?; }
        Ok(())
    }
}
