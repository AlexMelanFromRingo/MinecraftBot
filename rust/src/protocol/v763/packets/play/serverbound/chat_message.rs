...
impl ChatMessage {
    ...
    pub fn decode(reader: &mut BytesReader<'_>) -> Result<Self, ProtocolError> {
        ...
        let timestamp = i64::from_be_bytes([__buf[0],__buf[1],__buf[2],__buf[3],__buf[4],__buf[5],__buf[6],__buf[7]]);
        let salt = i64::from_be_bytes([__buf[8],__buf[9],__buf[10],__buf[11],__buf[12],__buf[13],__buf[14],__buf[15]]);
        let present = reader.read_exact(1)?[0];
        let signature = match present {
            0 => None,
            1 => Some(reader.read_exact(256)?.to_vec()),
            other => return Err(ProtocolError::DecodeError(format!("signature.present: {}", other))),
        };
