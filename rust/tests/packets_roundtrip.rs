...
use minecraft_bot::codec::{varint, BytesReader, BytesWriter};
...
use minecraft_bot::protocol::v763::packets::login::clientbound::{
    compress::Compress,
    disconnect::Disconnect as LoginDisconnect,
...
use minecraft_bot::protocol::v763::packets::play::serverbound::{
    keep_alive::KeepAlive as SbKeepAlive, teleport_confirm::TeleportConfirm,
};
...
#[test]
fn login_disconnect() {
    let pkt = LoginDisconnect {
        reason: "{\"text\":\"banned\"}".into(),
    };
...
fn play_keep_alive_clientbound() {
    let pkt = CbKeepAlive {
        keep_alive_id: 0x1122_3344_5566_7788_u64 as i64,
    };
    let bytes = round_trip_clientbound(&pkt);
    let mut r = BytesReader::new(&bytes);
...
fn play_keep_alive_serverbound() {
    let pkt = SbKeepAlive {
        keep_alive_id: 0x4242_4242_4242_4242_u64 as i64,
    };
    let bytes = round_trip_serverbound(&pkt);
    let mut r = BytesReader::new(&bytes);
...
fn play_select_advancement_tab_with_id() {
    let pkt = SelectAdvancementTab {
        id: Some("minecraft:story/root".into()),
    };
    let bytes = round_trip_clientbound(&pkt);
    let mut r = BytesReader::new(&bytes);
